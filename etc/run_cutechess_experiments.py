...
stockfish_nodes_options = [None]  # [3200, 6400, 12800]
...
def generate_and_run_command(batch_size, movetime, nodes, stockfish_nodes=None):
    start_time = datetime.now()
    if stockfish_nodes is None:
    ...
    with open(f'{pgnout[:-4]}_output.txt', 'w') as f:
        print("=====================================")
        new_experiment_info = f"New Experiment: BS: {batch_size}, Movetime: {movetime}, Nodes: {nodes}, Stockfish Nodes: {stockfish_nodes}\n"
...
for batch_size in batch_size_options:
    for nodes in nodes_options:
        movetime = 0
        for stockfish_nodes in stockfish_nodes_options:
            generate_and_run_command(batch_size, movetime, nodes, stockfish_nodes=stockfish_nodes)
